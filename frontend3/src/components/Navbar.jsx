        ...
        <div className='flex items-center justify-between py-5 font-medium'>
            ...
            <div className='flex items-center gap-6'>
                ...
                <div className='group relative'>
                    {/* <Link onClick={()=> token ? null : navigate('/login')}>
                ...
                <img src={assets.cart_icon} className='w-5 min-w-5' alt="" /><p className='absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4 bg-black text-white aspect-square rounded-full text-[8px]'>{getCartCount()}</p></Link>
                <img onClick={()=> setVisible(true)} src={assets.menu_icon} className='w-5 cursor-pointer sm:hidden' alt="" />
            </div>
            {/* SideBAr menu for scamll screen */}
            <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'}`}>
                <div className='flex flex-col text-gray-600'>
                    ...
                    <NavLink className="py-2 pl-6 border border-gray-100" onClick={()=> setVisible(false)} to={'/'}>Home</NavLink> 
                    <NavLink className="py-2 pl-6 border border-gray-100" onClick={()=> setVisible(false)} to={'/collection'}>Collection</NavLink> 
                    <NavLink className="py-2 pl-6 border border-gray-100" onClick={()=> setVisible(false)} to={'/about'}>About</NavLink> 
                    <NavLink className="py-2 pl-6 border border-gray-100" onClick={()=> setVisible(false)} to={'/contact'}>Contact</NavLink>
                </div>
            </div>
        </div>
    </>
